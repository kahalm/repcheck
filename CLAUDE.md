...
- **chess.com-Züge kommen von chess.com selbst** (v1.65.0, `lib/chesscom-moves.js`) — nicht mehr nur aus der Zugliste im DOM. `GET /callback/{live|daily}/game/{id}` (derselbe Abruf, der schon die Kopfzeilen holt) liefert `game.moveList` als **TCN**: zwei Zeichen je Halbzug, Feldnummer 0–63 über ein festes Alphabet; ein zweites Zeichen > 63 heißt Umwandlung (Figur aus `qnrbkp`, dazu links/geradeaus/rechts). `sansFromTcn` spielt die Züge auf einem chess.js-Brett nach und gibt SAN zurück. **Jeder Fehler ergibt eine LEERE Liste** (ungerade Länge, unbekanntes Zeichen, unspielbarer Zug, Crazyhouse-Einsetzzug) — eine halb gelesene Partie liefe sonst unbemerkt in den Import; dann greift weiter die DOM-Auslese. Anlass: auf `/analysis/game/<id>/review` rendert chess.com **gar keine** Zugliste (Dump 2026-09-24: 0 `.node`-Knoten), und Partie-Speichern tat dort nichts (bis v1.64.1 sogar stumm). Test-Vektor ist eine echte Partie: 94 Zeichen → genau die 47 Züge, die der Analyse-Tab derselben Partie zeigt.
...
- **„Kurs nicht im Konto" ist kein Alarm** (v1.65.1) — antwortet Chessable mit `{"error":{"code":"BOOK_NOT_OWNED","data":{"name":…}}}` (getCourse, HTTP 400: das eingeloggte Konto besitzt den Kurs nicht), zweigt `chessableGetChecked` VOR dem Alarm-Pfad ab: `checkChessableResponse` liefert dafür `code` + `courseName`, `showNotOwned` zeigt eine Karte „Kurs nicht in deinem Chessable-Konto" mit Prüfhinweisen (richtiges Konto eingeloggt? unter „My Courses"? mehrteilige Kurse = je Teil eine eigene Seite), der Status sagt dasselbe. Kein `rcCrawlAlert`, kein Discord-Aufruf, keine Meldung an RookHub. Alte Hinweise aus v1.64.x mit diesem Code räumt das Popup still weg (`isNotOwnedAlert`), sonst fragte es vor dem nächsten Holen weiter nach dem Entwickler. Anlass: 24.09.2026 acht Versuche in sechs Minuten an „Lifetime Repertoires: King's Indian Defense - Part 1" (bid 77656), jeder mit der Bitte, dem Entwickler Bescheid zu geben; am 22.09. derselbe Code bei „Blohberger's Nimzo-Ragozin – Part 1".
